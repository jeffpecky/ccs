...
describe('formatErrorMessage', () => {
  ...
  test('UNKNOWN preserves backward-compat 3-bullet feel and ends with verbose hint', () => {
    ...
    expect(lines.some((l) => l.startsWith('Try: Authenticate codex from the dashboard'))).toBe(
      true
    );
...
describe('cross-profile OAuth failure matrix', () => {
  ...
  test.each(providers.flatMap((provider) => matrix.map((scenario) => ({ provider, scenario }))))(
    ...
    ({ provider, scenario }) => {
      ...
      expect(
        message.includes(`Authenticate ${provider} from the dashboard`) ||
          message.includes('Update from the dashboard settings')
      ).toBe(true);
    }
  );
});
