...
export function windowsBarAssetName(arch: string): string {
  if (arch !== 'x64')
    throw new Error(`Unsupported Windows architecture: ${arch}. CCS Bar requires win-x64.`);
...
async function fetchWindowsAsset(): Promise<Asset> {
  ...
  if (!response.ok)
    throw new Error(`GitHub API returned ${response.status} for tag ${RELEASE_TAG}`);
  ...
  if (!asset || !digest?.[1])
    throw new Error(`Release asset ${WINDOWS_ASSET} with SHA-256 digest not found.`);
  return { downloadUrl: asset.browser_download_url, sha256: digest[1].toLowerCase() };
}

async function stageWindowsAsset(
  url: string,
  staging: string,
  ...
  if (
    ...
    !['github.com', 'objects.githubusercontent.com'].includes(parsed.hostname)
  ) {
...
function registerWindowsShortcut(shortcut: string, target: string): void {
  ...
  const script =
    '$w=New-Object -ComObject WScript.Shell;$s=$w.CreateShortcut($args[0]);$s.TargetPath=$args[1];$s.WorkingDirectory=(Split-Path $args[1]);$s.Save()';
  try {
    execFileSync(
      'powershell.exe',
      ['-NoProfile', '-NonInteractive', '-Command', script, shortcut, target],
      { stdio: 'pipe' }
    );
  } catch (error) {
    throw new Error(
      `Shortcut registration failed: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function isWindowsBarRunning(): Promise<boolean> {
  const script =
    '(Get-CimInstance Win32_Process | Where-Object {$_.ExecutablePath -eq $args[0]} | Select-Object -First 1).ProcessId';
  const { stdout } = await execFileAsync('powershell.exe', [
    '-NoProfile',
    '-NonInteractive',
    '-Command',
    script,
    getWindowsBarPaths().exe,
  ]);
...
export async function installWindowsBar(
  args: string[],
  supplied: Partial<WindowsInstallDeps> = {}
): Promise<void> {
  ...
  const registrations = [
    ...
    paths.startupShortcut,
  ];
  const priorRegistrations = new Map(
    registrations.map((file) => [file, fs.existsSync(file) ? fs.readFileSync(file) : null])
  );
  let swapped = false;
  try {
    const asset = await deps.fetchAsset();
    await deps.stageAsset(asset.downloadUrl, staging, asset.sha256);
    if (!fs.existsSync(path.join(stagedApp, 'CCS Bar.exe')))
      throw new Error('Package missing CCS Bar/CCS Bar.exe.');
    if (args.includes('--await-quit')) {
      const deadline = Date.now() + 15_000;
      while (await deps.appRunning()) {
        if (Date.now() >= deadline)
          throw new Error('CCS Bar is still running. Quit it and retry update.');
        await sleep(300);
      }
    }
    ...
    try {
      deps.rename(stagedApp, paths.installDir);
    } catch (error) {
    ...
    if (version === 'unknown') {
      try {
        const manifest = JSON.parse(
          fs.readFileSync(path.join(paths.installDir, 'manifest.json'), 'utf8')
        ) as { version?: string };
        version = manifest.version?.trim() || version;
      } catch {
        /* package validation already checked executable */
      }
    }
    ...
    if (swapped) {
      ...
      for (const artifact of registrations) {
        ...
        if (prior) {
          ...
          fs.writeFileSync(artifact, prior);
        }
      }
    }
    console.error(
      `[X] CCS Bar install failed: ${error instanceof Error ? error.message : String(error)}`
    );
...
export async function uninstallWindowsBar(
  ...
  try {
    await (
      ...
      (async () => {
        const script =
          'Get-CimInstance Win32_Process | Where-Object {$_.ExecutablePath -eq $args[0]} | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }';
        execFileSync(
          'powershell.exe',
          ['-NoProfile', '-NonInteractive', '-Command', script, paths.exe],
          { stdio: 'pipe' }
        );
      })
    )();
    ...
    console.error(
      `[X] CCS Bar uninstall failed: ${error instanceof Error ? error.message : String(error)}`
    );
    if (Object.keys(supplied).length === 0) process.exitCode = 1;
  }
}
